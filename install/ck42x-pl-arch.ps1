...
try {
  ...
  $pkg = Get-ChildItem $tmp -Directory | Where-Object { $_.Name -like 'ck42x-pl-arch*' } | Select-Object -First 1
  if (-not $pkg) { throw 'Invalid package layout' }
  if (Test-Path $InstallRoot) { Remove-Item $InstallRoot -Recurse -Force }
  Copy-Item $pkg.FullName $InstallRoot -Recurse
} catch {
